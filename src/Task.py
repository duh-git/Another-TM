class Task:
  ...
  def __str__(self):
    ...
    formatted_description = f"{self.description:^32}" if len(self.description) < 32 else f"{self.description:.29}..."
...
if __name__ == "__main__":
  ...
  print("\nFind task by list id (not by task.id) index", tasks.index(task_for_serching_or_deleting))
